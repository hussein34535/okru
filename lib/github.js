...
export async function getLatestInProgressRun() {
  ...
  const runsUrl = `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/actions/runs?per_page=30`;
  ...
  async function fetchRunsOnce() {
    ...
    if (data.workflow_runs && data.workflow_runs.length > 0) {
      // Find the latest run with an active status
  ...
  // Brief retry loop to handle propagation delay after dispatch
  const maxAttempts = 4; // ~3 seconds total
  ...
  return null; // No active runs found
}
